...
const CreateResumeForm = ({ onSuccess }) => {   // ✅ accept onSuccess
  ...
  const handleCreateResume = async (e) => {
    ...
    try {
      ...
      if (response.data?._id) {
        if (onSuccess) {
          onSuccess(response.data)   // ✅ call Dashboard refresh
        } else {
          navigate(`/resume/${response.data?._id}`)  // fallback if no onSuccess
        }
      }
    } catch (error) {
    ...
    <div className='w-full max-w-md p-8 bg-white rounded-2xl border border-gray-100 shadow-lg'>
      ...
      <form onSubmit={handleCreateResume}>
        <Input
          ...
          label='Resume Title'
          placeholder='e.g., Saurav Patel - Software Engineer'
          type='text'
        />
